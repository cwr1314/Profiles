...
(() => {
  ...
  if (magicJS.isResponse) {
    switch (true) {
      // 回答内容优化
      case magicJS.read("zhihu_settings_answer_tip") != true &&
      ...
      case /^https?:\/\/api\.zhihu\.com\/(answers|pins|comments?|articles)\/\d+\/(root|child)_comments/.test(magicJS.request.url):
        ...
        break;
      // 知乎热榜去广告
      case magicJS.read("zhihu_settings_hot_list") != true && /^https?:\/\/api\.zhihu\.com\/topstory\/hot-lists?(\?|\/)/.test(magicJS.request.url):
        response = removeHotListAds();
        break;
      // 拦截官方账号推广消息
      case magicJS.read("zhihu_settings_sys_msg") != true && /^https?:\/\/api\.zhihu\.com\/notifications\/v3\/timeline\/entry\/system_message/.test(magicJS.request.url):
        response = removeSysMsgAds();
        break;
      // 屏蔽官方营销消息
      case magicJS.read("zhihu_settings_sys_msg") != true && /^https?:\/\/api\.zhihu\.com\/notifications\/v3\/message/.test(magicJS.request.url):
      ...
      case magicJS.read("zhihu_settings_hot_search") == true && /^https?:\/\/api\.zhihu\.com\/search\/top_search\/tabs\/hot\/items/.test(magicJS.request.url):
        ...
        break;
      // 黑名单管理
      case magicJS.read("zhihu_settings_blocked_users") != true && /^https?:\/\/api\.zhihu\.com\/settings\/blocked_users/.test(magicJS.request.url):
        manageBlackUser();
        break;
      default:
        break;
    ...
    // 知乎屏蔽关键词解锁
    if (magicJS.read("zhihu_settings_blocked_keywords") != true && /^https?:\/\/api\.zhihu\.com\/feed-root\/block/.test(magicJS.request.url) === true) {
